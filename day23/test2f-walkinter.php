...
function walk($node = 0, $from = null, $done = [], $len = 0) {
	...
	while (count($nodes[$node]) < 3) {
		...
		$next = null; foreach ($nodes[$node] as $link) if ($link !== $from) { $next = $link; break; }
		// fin du parcours si aucun noeud à suivre (impasse)
		if ($next === null) return;
		// avance au noeud suivant...
		$from = $node; $node = $next; $len++;
	}
