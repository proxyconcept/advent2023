...
function run_button() {
	...
	while ($action = array_shift($todo)) {
		if ($action[1] === false && $action[0] === 'rx') return false; // stop cycle (état final trouvé)
		
#		$todo = array_merge($todo, run_module($action[0], $action[1], $action[2]));  // +0.2s/10000
		foreach (run_module($action[0], $action[1], $action[2]) as $a) $todo[] = $a;
	}
