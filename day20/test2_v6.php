...
function run_button() {
	...
	while ($todo) {
		...
		foreach ($todo as $action) {
			...
			if ($modules[$name]['type'] === '%') {  // soit un module flip-flop
				if ($pulse) continue;
				$pulse = $modules[$name]['status'] = ! $modules[$name]['status'];
			} else {                                // soit un module flip-flop
				$modules[$name]['status'][$from] = $pulse;
#				$pulse = in_array(false, $modules[$name]['status']);
#				$pulse = false; foreach ($modules[$name]['status'] as $state) if (! $state) { $pulse = true; break; }
#				$pulse = ($pulse) ? in_array(false, $modules[$name]['status']) : true;
				if ($pulse) { $pulse = false; foreach ($modules[$name]['status'] as $state) if (! $state) { $pulse = true; break; } } else $pulse = true;
			}
			// aucun autre cas de module possible (broadcaster traité initialement)
			foreach ($modules[$name]['out'] as $out) {
